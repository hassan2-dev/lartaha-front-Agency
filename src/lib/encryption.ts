...
export async function encryptFileChunked(
  ...
  const totalEncryptedSize =
    headerSize + encryptedChunks.reduce((sum, chunk) => sum + chunk.byteLength, 0)
...
export async function decryptFileChunked(
  ...
  if (input.encryptedChunks.length > 1) {
    ...
    if (
      headerData.length >= magicBytes.length &&
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        try {
          ...
          throw new Error(
            `Failed to decrypt chunk ${i}: ${err instanceof Error ? err.message : String(err)}`
          )
        }
...
async function generateVideoThumbnail(file: File, maxSize: number): Promise<Blob | null> {
  ...
  if (file.size > MAX_VIDEO_SIZE) {
    console.log(
      '[DEBUG] generateVideoThumbnail: skipping large video',
      file.name,
