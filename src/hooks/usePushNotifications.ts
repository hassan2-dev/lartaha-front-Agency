...
export function usePushNotifications() {
  ...
  useEffect(() => {
    ...
    console.log('🔔 [usePushNotifications] serviceWorker in navigator:', 'serviceWorker' in navigator);
    ...
    if (basicSupport) {
      console.log('🔔 [usePushNotifications] Basic support detected, checking service worker readiness...');
      setIsSupported(true);

      // Check if service worker is already registered and ready
      navigator.serviceWorker.getRegistration().then(registration => {
        if (registration) {
          console.log('🔔 [usePushNotifications] Service worker already registered:', registration.scope);
          checkSubscription();
        } else {
          console.log('🔔 [usePushNotifications] No service worker registration found yet, will check after registration');
        }
      }).catch(error => {
        console.error('🔔 [usePushNotifications] Error checking service worker registration:', error);
      });

      // Listen for service worker registration changes
      const handleSWChange = () => {
        console.log('🔔 [usePushNotifications] Service worker change detected, checking subscription...');
        checkSubscription();
      };
      ...
      console.log('🔔 [usePushNotifications] Push notifications NOT supported, setting isSupported to false');
  ...
  const subscribe = useCallback(async () => {
    try {
      ...
      if (error.name === 'AbortError' && error.message?.includes('push service error')) {
        console.error('🔔 [usePushNotifications] Push service error - this usually means VAPID keys are invalid or mismatched');
        alert('Push notification service error. This could be due to invalid VAPID configuration. Please try refreshing the page or contact support.');
