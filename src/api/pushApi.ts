...
function validateVapidKey() {
  ...
  const isValidLength =
    NORMALIZED_VAPID_PUBLIC_KEY.length === 87 ||
    NORMALIZED_VAPID_PUBLIC_KEY.length === 65;
  if (!isValidLength) {
    console.error('🔔 [pushApi] VAPID public key appears to be invalid (wrong length):', NORMALIZED_VAPID_PUBLIC_KEY.length);
    console.error('🔔 [pushApi] Expected 87 or 65 characters, got:', NORMALIZED_VAPID_PUBLIC_KEY.length);
    return false;
  }

  console.log('🔔 [pushApi] VAPID public key validation passed, length:', NORMALIZED_VAPID_PUBLIC_KEY.length);
  return true;
}
      ...
      try {
        ...
        if (sub) {
          try {
            ...
            console.warn('🔔 [pushApi] Failed cleanup while resetting registration:', unsubscribeError);
          }
        }
      } finally {
...
export async function subscribeUserToPush() {
  ...
  console.log('🔔 [pushApi] VAPID public key configured:', NORMALIZED_VAPID_PUBLIC_KEY.substring(0, 20) + '...');
  ...
  if (existingSubscription) {
    ...
    const existingKey = existingKeyBuffer
      ? new Uint8Array(existingKeyBuffer)
      : null;
    const keyMatches = existingKey
      ? areUint8ArraysEqual(existingKey, applicationServerKey)
      : false;
  ...
  try {
    ...
    console.warn('🔔 [pushApi] Initial subscription failed, retrying after cleanup:', error?.message || error);
    await cleanupExistingSubscription(registration);
    ...
    try {
      ...
      const isStillRecoverable =
        retryError?.name === 'AbortError' || retryError?.name === 'InvalidStateError';

      if (!isStillRecoverable) {
        throw retryError;
      }

      console.warn('🔔 [pushApi] Retry after cleanup failed, performing hard SW reset:', retryError?.message || retryError);
